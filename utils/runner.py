...
class Runner:
    ...
    def train(self):
        ...
        for iter_cnt in pbar:
            ...
            #loss = self.loss()
            loss_ce = self.loss_ce(out_label, l_t)
            loss_ce += self.loss_ce(rec_label, l_s)
            ...
            loss = loss_ce + loss_rec*10 + loss_lpips.mean()
            ...
            pbar.set_postfix_str(f'total_loss:{loss.item():.4f}, ce_loss:{loss_ce.item():.4f}, rec_loss:{loss_rec.item():.4f}, lpips_loss:{loss_lpips.mean().item():.4f}')
        del pbar
